...
public class WEBFunctionality_All extends TestBase{
	...
	public void WEBFunctionality_Landing () throws Exception {
		...
		VIPLandingPage.contents.Attributes6SS.verifyText("Available for Telecom, CG&S, Retail, Travel, and Oil & Gas Industries");
		...
		VIPOverviewPage.contents.DownloadReportSaveButton.waitForExist(true, 20);
...
//		WebControl.takeScreenshot("WEB_Reg_Auto_063");
		...
		WebControl.waitForPageToLoad(45);
		Thread.sleep(2500);
		VIPLandingPage.contents.AddButton.waitForExist(true, 10);
...
//		VIPOverviewPage.contents.DownloadReportModalHeader.verifyDisplayed(true, 5);
//		Thread.sleep(2000);
//		VIPOverviewPage.contents.DownloadReportSaveButton.waitForExist(true, 45);
//		VIPOverviewPage.contents.DownloadReportSaveButton.click();
//		Thread.sleep(3000);}
...
//		WebControl.takeScreenshot("WEB_Reg_Auto_088");
		...
		WebControl.waitForPageToLoad(45);
		Thread.sleep(2000);
		VIPOverviewPage.contents.OverviewTitle.waitForExist(true, 15);
		...
		WebControl.waitForPageToLoad(45);
		Thread.sleep(3000);
		VIPLandingPage.contents.CompanyCardName.waitForExist(true, 35);
...
//		WebControl.takeScreenshot("WEB_Reg_Auto_093");
	...
	public void WEBFunctionality_Overview () throws Exception {
		...
		Thread.sleep(1500);
		VIPFinancialsPage.contents.ScoreCardDate.waitForExist(true, 10);
		...
		if (ROICTrendValue.substring(0, 1).equals("-")) {
			...
			VIPOverviewPage.contents.KeyInsights3.verifyText("Return on Invested Capital decreased by " + ROICTrendValue.replace("-", "") + " year-on-year and is at " + ROICValue + " for the trailing 12-month period (" + PeriodYr + ")");}
		}
		else {
			if (Period.equals("TTM")) {
			VIPOverviewPage.contents.KeyInsights3.verifyText("Return on Invested Capital increased by " + ROICTrendValue.replace("-", "") + " year-on-year and is at " + ROICValue + " for the trailing 12-month period (TTM)");}
			else {
			VIPOverviewPage.contents.KeyInsights3.verifyText("Return on Invested Capital increased by " + ROICTrendValue.replace("-", "") + " year-on-year and is at " + ROICValue + " for the trailing 12-month period (" + PeriodYr + ")");}
		}
		...
		else {VIPOverviewPage.contents.KeyInsights5.verifyText("Company’s capital efficiency improved by " + CETrendValue.replace("-", "") + " in the last one year to " + CEValue + " for the trailing 12-month period");}
...
//		WebControl.takeScreenshot("WEB_Reg_Auto_117");
	...
	public void WEBFunctionality_Transcripts () throws Exception {
		...
		VIPOverviewPage.contents.GenerateFinancialReportButton.waitForExist(true, 10);
		Thread.sleep(2000);
		VIPTranscriptPage.contents.SideMenuGuidanceIntentTranscript.waitForExist(true, 10);
		VIPLandingPage.contents.HelpIcon.hover();
		VIPTranscriptPage.contents.SideMenuGuidanceIntentTranscript.click();
		VIPLandingPage.contents.HelpIcon.hover();
		VIPTranscriptPage.contents.TranscriptsTab.waitForExist(true, 20);
		VIPTranscriptPage.contents.TranscriptsTab.click();
...
//		String Speech = SpeechBody2[1];
		...
		if (VIPTranscriptPage.contents.Analysts1.isDisplayed()) {
		...
		if (VIPTranscriptPage.contents.TranscriptSpeech.isDisplayed()) {
			...
			if (count1 >= 750) {
				...
				VIPTranscriptPage.contents.FirstMoreLink.verifyDisplayed(false);
			}
		}
		
		if (VIPTranscriptPage.contents.TranscriptSpeech2.isDisplayed()) {
		...
		if (VIPTranscriptPage.contents.TranscriptSpeech3.isDisplayed()) {
...
//		WebControl.takeScreenshot("WEB_Reg_Auto_180");
	...
	public void WEBFunctionality_Architecture () throws Exception {
		...
		VIPLandingPage.contents.HelpES1.verifyText("VIP Enablement Session Q2 FY21");
		VIPLandingPage.contents.HelpES2.verifyText("VIP 2.0 Recorded Enablement Session");
		VIPLandingPage.contents.HelpES3.verifyText("VIP 3.0 Recorded Enablement Session");
		VIPLandingPage.contents.HelpES4.verifyText("VIP COVID-19 Recorded Enablement Session");
